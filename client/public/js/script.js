    ...
    $(function () {
      ...
      function initLightGallery(itemsToInit, addClass) {
        if (!isNoviBuilder) {
          $(itemsToInit).lightGallery({
            ...
            pause:
              parseInt($(itemsToInit).attr("data-lg-autoplay-delay"), 10) ||
              5000,
            addClass: addClass,
      ...
      if (plugins.accordion.length) {
        for (let i = 0; i < plugins.accordion.length; i++) {
          let items = plugins.accordion[i].querySelectorAll(".accordion-item"),
          ...
          for (let i = 0; i < items.length; i++) {
            ...
            let tmpFunction = function (item) {
              let head = items[i].querySelector(".accordion-head"),
              ...
              MultiSwitch({
                ...
                isolate:
                  plugins.accordion[i].querySelectorAll(".accordion-head"),
                state: items[i].classList.contains("active"),
                event: click,
              });

              if (!body.multiSwitchTarget.groups.active.state)
